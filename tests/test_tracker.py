...
def test_check_stock_price_change_threshold_met(mock_finnhub_client, mock_db_manager, mock_ticker_queue,
    ...
    check_stock_price_change(ticker_config, mock_ticker_queue, mock_finnhub_client,
                             mock_db_manager, 60, 10)
...
def test_check_stock_price_change_neg_threshold_met_partially(mock_finnhub_client, mock_db_manager, mock_ticker_queue,
    ...
    check_stock_price_change(ticker_config, mock_queue, mock_finnhub_client,
                             mock_db_manager, 60, 10)
...
def test_check_stock_price_change_cooldown_active(mock_finnhub_client, mock_db_manager, mock_ticker_queue,
                                                  mock_send_notification):
    ticker_config = {
        'AAPL': [{'value': 2.0, 'users': [1]}]
    }
    ...
    mock_finnhub_client.quote.return_value = {'c': 150, 'pc': 140, 'dp': 7.14}
    mock_db_manager.get_user_notification_count.return_value = 0
    mock_db_manager.get_ticker_state.return_value = (True, datetime.utcnow() - timedelta(minutes=59))
...
def test_check_stock_price_change_resets_after_exceeding_limit(mock_finnhub_client, mock_db_manager, mock_ticker_queue,
    ...
    mock_db_manager.get_ticker_state.return_value = (False, datetime.utcnow() - timedelta(minutes=90))
    ...
    check_stock_price_change(ticker_config, mock_ticker_queue, mock_finnhub_client,
                             mock_db_manager, cooldown_minutes=60, max_notifications=10)
...
def test_check_stock_price_change_after_cooldown_active(mock_finnhub_client, mock_db_manager, mock_ticker_queue,
                                                        mock_send_notification):
    ticker_config = {
        'AAPL': [{'value': 2.0, 'users': [1]}]
    }
    ...
    mock_finnhub_client.quote.return_value = {'c': 150, 'pc': 140, 'dp': 7.14}
    mock_db_manager.get_user_notification_count.return_value = 0
    mock_db_manager.get_ticker_state.return_value = (True, datetime.utcnow() - timedelta(minutes=60))
